...
func (s *UserAuthTokenService) deleteExpiredExternalSessions(ctx context.Context, maxInactiveLifetime, maxLifetime time.Duration) (int64, error) {
	createdBefore := getTime().Add(-maxLifetime)
	rotatedBefore := getTime().Add(-maxInactiveLifetime)
	...
	var affected int64
	err := s.sqlStore.WithDbSession(ctx, func(dbSession *db.Session) error {
		sql := `DELETE FROM external_session es WHERE NOT EXISTS (SELECT 1 FROM user_auth_token uat WHERE es.id = uat.external_session_id)`
		res, err := dbSession.Exec(sql, createdBefore.Unix(), rotatedBefore.Unix())
		if err != nil {
			return err
		}
