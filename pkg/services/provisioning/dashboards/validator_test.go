...
func TestDuplicatesValidator(t *testing.T) {
	...
	t.Run("Duplicates validator should collect info about duplicate UIDs and titles within folders", func(t *testing.T) {
		...
		reader1, err := NewDashboardFileReader(cfg1, logger, nil, fakeStore, featuremgmt.WithFeatures(), nil)
		reader1.dashboardProvisioningService = fakeService
		require.NoError(t, err)

		reader2, err := NewDashboardFileReader(cfg2, logger, nil, fakeStore, featuremgmt.WithFeatures(), nil)
	...
	t.Run("Duplicates validator should restrict write access only for readers with duplicates", func(t *testing.T) {
		...
		reader3, err := NewDashboardFileReader(cfg3, logger, nil, fakeStore, featuremgmt.WithFeatures(), nil)
		...
		r, err = NewDashboardFileReader(cfg3, logger, nil, fakeStore, featuremgmt.WithFeatures(), nil)
		require.NoError(t, err)
