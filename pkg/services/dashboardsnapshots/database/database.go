...
func (d *DashboardSnapshotStore) SearchDashboardSnapshots(ctx context.Context, query *dashboardsnapshots.GetDashboardSnapshotsQuery) (dashboardsnapshots.DashboardSnapshotsList, error) {
	...
	err := d.store.WithDbSession(ctx, func(sess *db.Session) error {
		...
		var userID int64
		if identity.IsIdentityType(query.SignedInUser.GetID().String(), identity.TypeUser, identity.TypeServiceAccount) {
			var err error
			userID, err = identity.UserIdentifier(query.SignedInUser.GetID().String())
			if err != nil {
				return err
			}
		}
