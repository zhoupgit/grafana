...
func GetKerberosSettings() (kerberosAuth KerberosAuth) {
	kerberosAuth.Realm = os.Getenv("KRB5_REALM")
	kerberosAuth.UDPConnectionLimit = os.Getenv("KRB5_UDP_PREFERENCE_LIMIT")
	kerberosAuth.CredentialCacheLookupFile = os.Getenv("KRB5_CC_LOOKUP_FILE")
	kerberosAuth.EnableDNSLookupKDC = os.Getenv("KRB5_DNS_LOOKUP_KDC")
	kerberosAuth.KeytabFilePath = os.Getenv("KRB5_CLIENT_KTNAME")
	kerberosAuth.CredentialCache = os.Getenv("KRB5CCNAME")
	kerberosAuth.ConfigFilePath = os.Getenv("KRB5_CONFIG")
	return kerberosAuth
}
...
func Krb5ParseAuthCredentials(host string, port string, db string, user string, pass string, kerberosAuth KerberosAuth) string {
	...
	if kerberosAuth.ConfigFilePath == "" {
		kerberosAuth.ConfigFilePath = "/etc/krb5.conf"
	}
	...
	if krb5CacheCredsFile != "" {
		...
		krb5DriverParams += fmt.Sprintf("server=%s;database=%s;user id=%s;krb5-realm=%s;krb5-keytabfile=%s;", host, db, user, kerberosAuth.Realm, kerberosAuth.KeytabFilePath)
	} else if kerberosAuth.KeytabFilePath == "" {
		krb5DriverParams += fmt.Sprintf("server=%s;database=%s;user id=%s;krb5-realm=%s;password=%s;", host, db, user, kerberosAuth.Realm, pass)
	} else {
